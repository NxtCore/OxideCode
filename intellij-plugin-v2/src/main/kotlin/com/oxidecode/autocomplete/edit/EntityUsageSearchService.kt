...
class EntityUsageSearchService(
    ...
    private val numDefinitionsToFetch: Int = 6

    private val numUsagesToFetch: Int = 6
...
//        val endLine = startLine + maxOf(0, lines.size - 1)
    ...
    fun getCurrentLineEntityUsages(currentEditorState: EditorState): List<FileChunk> {
        ...
        try {
            ...
            val language = OxideCodeConstants.EXTENSION_TO_LANGUAGE[currentFileExtension]
            val relevantKeywords = language?.let { OxideCodeConstants.LANGUAGE_KEYWORDS[it] } ?: emptyList()

            val candidateTerms =
                searchText
                    .replace(Regex(OxideCodeConstants.COMMON_SYMBOLS_REGEX), " ") // Remove common symbols
                    .split("\\s+".toRegex())
                    .filter { term ->
                        term.length >= 3 &&
    ...
    private fun isLanguageKeyword(
        ...
        val language = OxideCodeConstants.EXTENSION_TO_LANGUAGE[fileExtension] ?: return false

        // Get the keywords for this language
        val keywords = OxideCodeConstants.LANGUAGE_KEYWORDS[language] ?: return false
