package com.oxidecode.autocomplete.edit
...
import com.oxidecode.settings.OxideCodeMetaData
...
private fun invokeLaterIfGatewayModeClient(
    ...
    if (OxideCodeConstants.GATEWAY_MODE == OxideCodeConstants.GatewayMode.CLIENT) {
...
class RecentEditsTracker(
    ...
    private val debouncer =
        Debouncer({ OxideCodeConfig.getInstance(project).getDebounceThresholdMs() }, scope, project) { processLatestEdit() }
    ...
    private fun attachListenerToEditor(editor: Editor) {
        ...
        currentCaretListener =
            CaretPositionChangedAdapter {
                ...
                val lastEditTime = recentEdits.lastOrNull()?.timestamp ?: 0
                val currentTime = System.currentTimeMillis()
                val cursorMovementThreshold = 60000L
    ...
    init {
        OxideCodeSettings.getInstance().runNowAndOnSettingsChange(project, this) {
            if (nextEditPredictionFlagOn &&
                editorFactoryListener == null &&
                OxideCodeConstants.GATEWAY_MODE != OxideCodeConstants.GatewayMode.HOST
            ) {
            ...
            if (OxideCodeConstants.GATEWAY_MODE == OxideCodeConstants.GatewayMode.HOST) {
                EditorActionsRouterService.getInstance()
                cleanupFocusTracking()
                consumerJob?.cancel()
                consumerJob = null
                    ...
                    acceptanceDisposable =
                        it.accept(editor).also { disposable ->
                            if (it is AutocompleteSuggestion.GhostTextSuggestion ||
                                ...
                                val metadata = OxideCodeMetaData.getInstance()
...
//                    }
    ...
    private fun shouldExcludeFromAutocomplete(filePath: String): Boolean {
        val exclusionPatterns = OxideCodeConfig.getInstance(project).getAutocompleteExclusionPatterns()
        ...
        try {
            ...
            val request =
                NextEditAutocompleteRequest(
                    ...
                    privacy_mode_enabled = OxideCodeConfig.getInstance(project).isPrivacyModeEnabled(),
            ...
            NotificationDeduplicationService.getInstance(project).showNotificationWithDeduplicationAndErrorReporting(
                ...
                notificationGroup = "OxideCode Notifications",
