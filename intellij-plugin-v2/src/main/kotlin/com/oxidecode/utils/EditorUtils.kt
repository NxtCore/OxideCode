...
import com.oxidecode.data.SelectedSnippet
...
fun readFile(
    ...
    val maxFileSize = OxideCodeConstants.MAX_FILE_SIZE_BYTES
...
fun relativePath(
    ...
    // Check if it's a non-project file managed by OxideCodeNonProjectFilesService
    if (OxideCodeNonProjectFilesService.getInstance(project).isAllowedFile(fullPath)) return fullPath
    return project.osBasePath?.let { basePath -> relativePath(basePath, fullPath) }
}
...
fun absolutePath(
    ...
    if (OxideCodeNonProjectFilesService.getInstance(project).isAllowedFile(relativePath)) return relativePath
...
fun getCurrentSelectedFile(project: Project): VirtualFile? {
    ...
    return FileEditorManager
        ...
        .selectedFiles
        .filterNot {
            OxideCodeConstants.diffFiles.contains(it.name)
        }.firstOrNull {
            OxideCodeNonProjectFilesService.getInstance(project).isAllowedFile(it.url) ||
                OxideCodeNonProjectFilesService.getInstance(project).isAllowedFile(it.path) ||
                (
...
fun openFileInEditor(
    ...
    val virtualFile =
        if (useAbsolutePath) {
            ...
            val nonProjectService = OxideCodeNonProjectFilesService.getInstance(project)
