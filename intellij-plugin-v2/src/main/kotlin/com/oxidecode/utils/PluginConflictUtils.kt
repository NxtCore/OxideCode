...
fun disableFullLineCompletionAndNotify(project: Project) {
    ...
    val conflictingPlugins =
        OxideCodeConstants.PLUGINS_TO_DISABLE
    ...
    val pluginNames =
        conflictingPlugins
            .map { pluginId ->
                OxideCodeConstants.PLUGIN_ID_TO_NAME[pluginId] ?: PluginManagerCore.getPlugin(pluginId)?.name ?: pluginId.idString
    ...
    showNotification(
        project = project,
        title = "Conflicting Autocomplete Plugins",
        body =
            "The following plugins have conflicting autocomplete suggestions: $pluginNames. " +
                "If you still see conflicting autocomplete suggestions, please disable these plugins manually in Settings > Plugins.",
        notificationGroup = "Conflicting Autocomplete Plugins",
    )
