...
export class ApiClient {
	...
	/**
	 * Gets the effective backend mode, falling back to HTTP if native is unavailable.
	 */
	private getEffectiveBackend(): BackendMode {
		const preferredBackend = config.backend;
		if (preferredBackend === "native" && !isNativeAvailable()) {
			console.log(
				"[OxideCode] Native backend requested but unavailable, falling back to HTTP",
			);
			return "http";
		}
	...
	async getAutocomplete(
		...
		if (isFileTooLarge(documentText) || isFileTooLarge(input.originalContent)) {
			console.log("[OxideCode] Skipping autocomplete request: file too large", {
		...
		if (backend === "native") {
			return this.getAutocompleteNative(input, signal);
		}
		return this.getAutocompleteHttp(input, signal);
	}

	/**
	 * Gets autocomplete using the native Rust core.
	 */
	private async getAutocompleteNative(
		...
		const promptStyle: PromptStyle = config.promptStyle;
		const completionEndpoint: CompletionEndpoint = config.completionEndpoint;

		// Cursor position (1-indexed for Rust core)
		const cursorLine = position.line;
		const cursorCol = position.character;

		try {
			const hint = await nativePredictNextEdit(
				provider,
				completionEndpoint,
				promptStyle,
				deltas,
			...
			const result = nesHintToAutocompleteResult(hint, documentText);
			if (!result) {
				return null;
			}
	...
	/**
	 * Converts RecentChange[] to NativeEditDelta[] for the native core.
	 */
	private convertRecentChangesToDeltas(
		...
		for (const change of changes) {
			...
			for (const line of lines) {
				...
				if (hunkMatch?.[2]) {
					lineNum = Number.parseInt(hunkMatch[2], 10) - 1;
					continue;
				}
	...
	/**
	 * Gets autocomplete using the HTTP local server.
	 */
	private async getAutocompleteHttp(
		input: AutocompleteInput,
		signal?: AbortSignal,
	): Promise<AutocompleteResult[] | null> {
		if (!this.localServer) {
			console.error(
				"[OxideCode] HTTP backend requested but no local server configured",
			);
			return null;
		}
		...
		if (!parsedRequest.success) {
			console.error(
				"[OxideCode] Invalid request data:",
		...
		try {
			...
			console.error("[OxideCode] Failed to start local server:", error);
			return null;
		}
		...
		try {
			...
			console.error("[OxideCode] Local API request failed:", error);
	...
	getDebugInfo(): string {
		const extensionVersion =
			vscode.extensions.getExtension("oxidecode.oxidecode")?.packageJSON
				?.version ?? "unknown";
		const backend = this.getEffectiveBackend();
			...
			const req = http.request(options, (res) => {
				...
				res.on("end", () => {
					if (res.statusCode !== 200) {
						console.error(
							`[OxideCode] Local request failed with status ${res.statusCode}: ${data}`,
						);
						finish(() =>
							reject(
