...
data class DirectAutocompleteProviderConfig(
...
class SweepSettings : PersistentStateComponent<SweepSettings> {
    ...
    var baseUrl: String = DEFAULT_SWEEP_URL
        get() =
            field
                .trim()
                .trimEnd('/')
                .ifBlank { SweepEnvironmentConstants.Defaults.DEFAULT_BASE_URL }
        set(value) {
            if (value != field) {
                field = value
                notifySettingsChanged()
    ...
    var directAutocompleteProvider: DirectAutocompleteProviderConfig = DirectAutocompleteProviderConfig()
        set(value) {
            field = value
            notifySettingsChanged()
        }
     ...
     * 1. A backend base URL is available, OR
     * 2. A GitHub token has been provided, OR
     * 3. An Anthropic API key has been provided
     */
    val hasBeenSet: Boolean
        get() =
            baseUrl.isNotBlank() ||
                githubToken != DEFAULT_GITHUB_TOKEN ||
                anthropicApiKey != DEFAULT_ANTHROPIC_API_KEY

    val isDirectAutocompleteProviderConfigured: Boolean
        get() =
            directAutocompleteProvider.baseUrl.trim().isNotBlank() &&
                directAutocompleteProvider.model.trim().isNotBlank()
