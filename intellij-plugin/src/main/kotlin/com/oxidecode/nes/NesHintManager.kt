...
object NesHintManager {
    ...
    private val activeFoldRegions: MutableList<com.intellij.openapi.editor.FoldRegion> = mutableListOf()
            ...
            for (group in additionGroups) {
                ...
                if (inlay != null) {
                    ...
                    editor.foldingModel.runBatchFoldingOperation {
                        val foldStartLine = docLine + 1
                        val foldEndLine = editStartLine + group.last()

                        if (foldStartLine < document.lineCount) {
                            val start = document.getLineStartOffset(foldStartLine)
                            val end = document.getLineEndOffset(foldEndLine.coerceAtMost(document.lineCount - 1))

                            val region = editor.foldingModel.addFoldRegion(start, end, "...")
                            region?.let {
                                it.isExpanded = false
                                activeFoldRegions.add(it)
                            }
                        }
                    }
                }
